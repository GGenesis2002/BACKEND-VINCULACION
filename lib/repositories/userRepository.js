...
const updateUser = async (userId, userData, parsedImage) => {
  ...
  // CAMBIO AQUÍ: Usa bucket.storage.from si 'bucket' es el cliente de Supabase
  const { data, error: uploadError } = await bucket.storage
    .from('fotos_sistema') 
  ...
  const { data: { publicUrl } } = bucket.storage
    .from('fotos_sistema')
